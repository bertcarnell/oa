...
namespace oacpp
{
    ...
    void GaloisField::GF_poly_sum(int p, int n, std::vector<int> p1, std::vector<int> p2, std::vector<int> & sum)
    {
    ...
    // TODO:  I am copying the entire object here, there may be a better way to do with with
    //  a row or column iterator in the matrix object since these are read only on p1 and p2
    void GaloisField::GF_poly_prod(int p, int n, std::vector<int> & xton, std::vector<int> p1, std::vector<int> p2, std::vector<int> & prod)
    /*
     ...
     */
    int GaloisField::GF_ready(GF & gf, int p, int n, std::vector<int> & xton)
    {
        size_t q;
        ...
        for (size_t i = 0; i < q; i++)
        {
            ...
            if (i > 0 && gf.inv[i] <= 0)
            {
                std::string msg = boost::str(boost::format("There is something wrong with the Galois field \n used for q=%d.  Element %d has no reciprocal.\n") % static_cast<int>(q) % static_cast<int>(i));
        ...
        for (size_t i = 0; i < q; i++)
        {
            ...
            if (i > 0 && gf.neg[i] <= 0)
            {
                std::string msg = boost::str(boost::format("There is something wrong with the Galois field \n used for q=%d.  Element %d has no negative. \n") % static_cast<int>(q) % static_cast<int>(i));
                throw std::runtime_error(msg.c_str());
            }
        }
    ...
    void GaloisField::GF_print(GF & gf)
    {
        int n, p, q;
